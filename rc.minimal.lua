...
globalkeys = gears.table.join(
   ...
   awful.key({ modkey }, "Pause", function()  suspend_system() end,
      {description = "suspend system", group = "system"}),
   awful.key({ modkey, "Control" }, "Pause", function()  hybrid_sleep_system() end,
      {description = "suspend+hibernate system", group = "system"}),
   awful.key({ modkey, Alt, "Shift", "Control" }, "F12",     restarter_with("rc.default.lua"),   {description="switch to default configuration", group="awesome"}))
-- }}}
...
naughty.notify({ preset = naughty.config.presets.critical,
		 ...
		 text = "Press same combination to return to previous" })
